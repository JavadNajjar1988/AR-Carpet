...
def main():
    ...
    model_ids = [f.removesuffix('.glb') for f in listdir(args.input_models) if
                 join(args.input_models, f).endswith('.glb')]
    if len(model_ids) < 1:
        print("ERROR: could not find any GLB files in {}".format(args.input_models))
        return

    select_lines = [_SELECT_LINE.format(model_id=f, selected_str=('selected' if f == _DEFAULT_RUG_ID else '')) for f in
                    model_ids]
